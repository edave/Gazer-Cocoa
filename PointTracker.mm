...
static OpenGazer::Point pointbetweenrects(const OpenGazer::Point &point, CvRect source, CvRect dest) {
    return OpenGazer::Point((point.x-source.x)*(double(dest.width)/source.width)+dest.x,
		 (point.y-source.y)*(double(dest.height)/source.height)+dest.y);

}

static vector<OpenGazer::Point> pointbetweenrects(const vector<OpenGazer::Point> &points,
			...
			const IplImage *frame) 
{
	...
	vector<OpenGazer::Point> temppoints;
...
int PointTracker::getClosestTracker(const OpenGazer::Point &point) {
...
void PointTracker::updatetracker(int id, const OpenGazer::Point &point) {
    currentpoints[id] = point.cvpoint32();
    synchronizepoints();
}

void PointTracker::addtracker(const OpenGazer::Point &point) {
