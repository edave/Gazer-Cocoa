...
int Targets::getCurrentTarget(OpenGazer::Point point) {
    ...
    vector<OpenGazer::Point>::iterator first = targets.begin();
    vector<OpenGazer::Point>::iterator last = targets.end();
...
CalTarget::CalTarget(OpenGazer::Point point, 
...
TrackerOutput::TrackerOutput(OpenGazer::Point gazepoint, OpenGazer::Point target, int targetid):
...
void GazeTracker::addExemplar(OpenGazer::Point point, 
...
//     for(int i=0; i<caltargets.size(); i++) {
// 	OpenGazer::Point p = caltargets[i].point;
...
void GazeTracker::update(const IplImage *image) {
    if (isActive()) {
	output.gazepoint = OpenGazer::Point(gpx->getmean(SharedImage(image, &ignore)), 
...
int GazeTracker::getTargetId(OpenGazer::Point point) {
    return targets->getCurrentTarget(point);
}

OpenGazer::Point GazeTracker::getTarget(int id) {
